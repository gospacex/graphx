...
func TestTuGraph_Tracing_Noop(t *testing.T) {
	defer Reset()
...
func TestTuGraph_Tracing_NewSpanName(t *testing.T) {
	defer Reset()
	...
	for _, s := range exp.GetSpans() {
		if s.Name == "graphx.tugraphx.new" {
			return
		}
	}
	t.Error("span graphx.tugraphx.new not found")
}
