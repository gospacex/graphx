...
func TestName_ReturnsCorrectString(t *testing.T) {
	if got := New().Name(); got != "graphx.janusgraph" {
		t.Errorf("Name() = %q, want graphx.janusgraph", got)
	}
}
...
func TestBuild_Success(t *testing.T) {
	cli, err := New().Build("inst", map[string]any{"config": map[string]any{"endpoint": "ws://localhost:8182"}})
	if err != nil {
...
func TestConcurrentBuild_Singleton(t *testing.T) {
	...
	for i := 0; i < 5; i++ {
		wg.Add(1)
		go func() { defer wg.Done(); cli, err := New().Build("inst", map[string]any{"config": map[string]any{"endpoint": "ws://localhost:8182"}}); if err == nil { cli.Close() } }()
	}
	wg.Wait()
}
